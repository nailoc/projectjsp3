...
<body>
	...
	<form method="get" action="itemlist.jsp">
		<p><input type="text" name="keyword" value=""><input type="submit" value="검색"><a href="cart.jsp">장바구니</a></p>
	</form>
	<p>
		<%
		...
		%>
			<a class="<%=bnameList.get(i).equals(bname) ? "active" : "" %>" href="itemlist.jsp?bname=<%=bnameList.get(i)%>&kind=<%=kind %>"><%=bnameList.get(i).toUpperCase() %> <%=bnameCnt[i] %></a>
		<%
			}
		%>
	...
	<p>
		...
		%>
			<a class="<%=sortList1[i].equals(sort) ? "active" : "" %>" href="itemlist.jsp?<%=selSort%>&sort=<%=sortList1[i]%>"><%=sortList2[i] %></a>
		<%
			}
		%>
	</p>
